...
class _WorkshopsScreenState extends State<WorkshopsScreen> {
  ...
  void _loadWorkshops() {
    // 1. Try to find the Workshops category in eventData
    final workshopsCat = eventData.firstWhere(
      (c) => c.title.toLowerCase().contains('workshop'),
      orElse: () => const MainCategory(title: 'Workshops', subCategories: []),
    );

    final List<Map<String, String>> loadedWorkshops = [];
    if (workshopsCat.subCategories.isNotEmpty) {
      for (var sub in workshopsCat.subCategories) {
        final event = sub.events.isNotEmpty ? sub.events.first : null;
        loadedWorkshops.add({
          'title': sub.title,
          'img': sub.imageAsset.isNotEmpty ? sub.imageAsset : 'assets/workshops/world-technocon-bioinformatics-cancer-research.webp',
          'url': event?.redirectUrl ?? '',
        });
      }
    }

    if (loadedWorkshops.isNotEmpty) {
      _workshops.clear();
      _workshops.addAll(loadedWorkshops);
    } else {
      // Fallback/hardcoded defaults if Firestore yields nothing yet
      _workshops.addAll([
        {
    ...
    _aspectRatios = List<double?>.filled(_workshops.length, null, growable: false);
